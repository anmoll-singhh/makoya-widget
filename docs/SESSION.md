...
## Session log
- 2026-06-23 (block 2): Resumed as CTO. Read all tracking docs (no re-audit). **Corrected a bad handoff assumption: the Supabase MCP was NOT connected** — it was registered (local scope) with a stale arg format (positional token `sbp_…` instead of `--project-ref=vgxvkegjnibvsngjrxqa` + `SUPABASE_ACCESS_TOKEN` env). Re-registered it correctly via `claude mcp`; now shows ✔ Connected. **MCP tools load only at session start, so the migration is applied in the NEXT session (founder chose: restart → apply via MCP).** Verified via Supabase Management API that the `leads` table does NOT exist yet and the access token is valid (HTTP 200). **Plugins evaluated** (founder ask): installed official `claude-md-management` (lightweight: `claude-md-improver` skill + `/revise-claude-md`, reinforces doc hygiene); **declined `claude-mem`** (heavy — needs Redis+Postgres+worker daemon; contradicts bootstrap ethos and duplicates the git-tracked doc-based memory). **Decisions locked:** (1) apply migration via MCP after restart; (2) then merge BOTH Phase 0 + Phase 1 to main (CI-green, additive); deploy stays gated on founder approval. No code changes this block; CI status unchanged (43 tests / typecheck 0).

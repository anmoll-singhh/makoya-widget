...
> - **🟢 11 migrations now APPLIED to prod** (added `settings`). Advisor still clean (2 intentional INFOs + leaked-pw toggle).
> - **⛔ Still before full go-live:** (1) **deploy** `apps/web` + the new widget bundle (`vercel --prod` / publish core.js) so routes + telemetry ship; (2) enable Supabase leaked-password toggle; (3) Stripe account + real plan prices for billing checkout/webhook.
> - **Next:** Wave 5 — billing **scaffold** from `docs/PRICING-STRATEGY-V3.1.md` (`plan_quotas` + typed `PlanCatalog` + lib; Stripe checkout/webhook deferred) + partners/white-label #14 (builds on tenancy).

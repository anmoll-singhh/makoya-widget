...
| **Canonical branch** | `main` @ **`1714ae8`** — pushed to origin (`0 0` ahead/behind), CI green (639 web + 33 widget + 10 telemetry). ⚠️ **`main` ≠ prod**: last deploy was block-25 `b537147` / `makoya-2da2afnji`; everything from block 26 onward (motion, i18n, batch-2 widget = **18** features, scalability foundation) is merged but **un-deployed**. ⚠️ `main` working tree has 3 uncommitted files (see config-drift note). |

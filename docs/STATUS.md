...
> - **⛔ Before full go-live (your calls):** (1) authorize applying the 2 Wave-5 migrations to prod; (2) **deploy** `apps/web` + publish the new widget `core.js` (`vercel --prod`) so routes + telemetry ship; (3) enable Supabase leaked-password toggle; (4) Stripe account + real plan prices to wire billing checkout/webhook (schema is ready).

...
|---|---|---|---|
| `C:\Users\ANMOL\Desktop\makoya` | `feat/phase-1-licensing` | **Widget license + domain gate (Phase 1)** | 🔶 built + CI-green on branch (1 commit on top of main); migration + merge-to-main founder-gated |
...
> **Block 10 note (data + licensing):** Founder **deleted all `sites`** (clean slate; `leads` untouched) — new sites to be onboarded fresh. **Phase 1 widget licensing gate** built on `feat/phase-1-licensing` (config endpoint enforces per-site `license_status` + `allowed_domains`, monitor→enforce via `WIDGET_ENFORCE`, ships OFF). **Two founder-gated actions before ship:** (1) apply `widget_licensing` migration to prod Supabase, (2) merge branch→main. Plan + honest threat-model: `docs/plans/PHASE-1-LICENSING.md` (Origin-lock = deterrent; signed-token wall + direct-`core.js` bypass = Phase 1.5).

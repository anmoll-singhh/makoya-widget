...
>
> **Last updated:** 2026-06-26 · **Updated by:** Claude (block 23 — v3.1 backend-readiness program: master plan + **Waves 1 & 2** merged to `main`, QA + security green per lane; 5 migrations WRITTEN but prod-apply is founder-gated/batched)
>
...
> - Totals so far: **5 migrations, ~16 new files, ~80 new tests**, CI green on `main` each wave; all security reviews GO.
> - **⛔ Founder actions before any of this goes live (batched, per founder choice):** (1) apply the 5 new migrations to the prod Supabase project in timestamp order, (2) deploy, (3) coordinator-owned `packages/widget` change so the widget actually EMITS heartbeat + analytics events.
> - **Next:** Wave 3 = org/team/roles **tenancy** (the foundational RLS refactor — runs ALONE, additive policies, highest-risk lane), then Wave 4 (statement generator + proof pack + settings), then Wave 5 (billing ⛔Stripe-gated + partners).

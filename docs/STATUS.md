...
>
> **Last updated:** 2026-06-26 · **Updated by:** Claude (block 23 — v3.1 backend-readiness program: master plan + **Waves 1, 2 & 3** merged to `main`, QA + security GO per lane; 7 migrations WRITTEN but prod-apply is founder-gated/batched)
>
...
> - **⛔ Founder actions before any of this goes live (batched, per founder choice):** (1) apply the 7 new migrations (`2026062612xxxx`–`170100`) to the prod Supabase project IN TIMESTAMP ORDER, (2) deploy, (3) coordinator-owned `packages/widget` change so the widget actually EMITS heartbeat + analytics events.
> - **Next:** Wave 4 (statement generator #8 + proof-of-effort pack #9 + settings/customize extras #6/#12 — parallel-safe), then Wave 5 (billing #11 ⛔Stripe-gated + partners #14, depends on Wave 3 tenancy).

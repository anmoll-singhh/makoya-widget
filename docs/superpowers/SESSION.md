...
- **Phase 0 (foundation) — DONE, merged to main, deployed.** (plan: `docs/superpowers/plans/2026-06-20-phase0-foundation.md`)
  ...
  - **✅ MIGRATION APPLIED to live DB (2026-06-20)** via direct Postgres (`db.vgxvkegjnibvsngjrxqa.supabase.co`, user `postgres`, DB password from user). All 5 new `site_config` columns verified present; WS2 QA confirmed new fields persist end-to-end.
...
- **WS2 (client dashboard customizer-first) — DONE, merged to main, deployed.** (plan: `docs/superpowers/plans/2026-06-20-ws2-client-dashboard.md`)
  - shadcn/ui initialized in apps/web (radix-nova/new-york, neutral base, lucide), reconciled with brand tokens (Inter/Sora kept; `--primary`/`--ring` mapped to brand indigo; circular `--font-sans` removed). Primitives: button select switch input label tabs dialog card tooltip table badge.
  - Customizer-first `/dashboard`: lands straight on the live widget Customizer for the user's pre-assigned site. Grouped controls (Appearance: primaryColor/launcherIcon/launcherSize/position · Features: toggle + ↑/↓ reorder of the 15 · Behavior: defaultProfile/defaultLanguage · Branding paid-gated: hideBranding/panelTitle/accessibilityStatementUrl). Big **live preview = real `/widget/core.js` in a sandboxed iframe** (debounced 300ms, auto-opens panel). Debounced autosave (700ms) via existing `PATCH /api/sites/[id]/config`. Multi-site → `?site=` switcher (Customizer keyed on activeSiteId to avoid stale/cross-site writes).
  - Separate **`/dashboard/report`** tab = ScanReport (score/issues + book-a-call CTA) + Re-scan (key remount). Tab nav added to dashboard layout. Account page kept.
  - Removed self-serve add: deleted AddSiteForm, ConfigEditor, `POST /api/sites`; `/dashboard/sites/[id]` redirects to `/dashboard?site=<id>`.
  - QA (live, cookie-mint auth): 11/11 PASS incl. DB-verified persistence (position→top-left, features 15→14, all 5 new columns), free-plan branding disabled, mobile + report screenshots. Built via subagent-driven-development (impl+review+fix per task, final opus whole-branch review).
- **WS3 (admin dashboard worst-first) — NOT STARTED.** Next: clients overview (email, domains, latestScore, issueCount, plan, openRequests) default sorted most-issues-first + sortable; Add-customer dialog (POST /api/admin/customers → handover payload); customer detail (plan change, scans, requests, config link). Data already in `listAdminSites()`. Plan: `docs/superpowers/plans/2026-06-20-ws3-admin-dashboard.md`. Use subagent-driven-development + design skills (keep dark operator theme; wrap admin in `.dark` for shadcn tokens). Prod QA test records AUTHORIZED.

...
function isActivatable(el: Element): boolean {
  ...
  // ARIA hidden is inherited: a button inside aria-hidden="true" is removed from
  // the a11y tree, so don't activate it (closest walks the ancestor chain).
...
export function makeVoiceNav(opts: { getLang: () => Lang }): { enable(): void; disable(): void } {
  ...
  function runCommand(raw: string): void {
    ...
    try {
      ...
      // Word-boundary so "laptop"/"desktop" don't trigger "top".
      if (/\btop\b/.test(cmd)) return void window.scrollTo({ top: 0, behavior: "smooth" });
      if (/\bbottom\b/.test(cmd)) return void window.scrollTo({ top: document.body.scrollHeight, behavior: "smooth" });
  ...
  return {
    enable() {
      ...
      try {
        ...
        // PERMANENT failures (mic denied) must stop the auto-restart, or browsers
        // that report 'not-allowed' ASYNChronously (Firefox/Safari) loop forever:
        // error → end → start → error. Flip `enabled` off so onend won't restart.
        rec.onerror = (e: SRErrorEvent) => {
          ...
          // other errors (e.g. "no-speech") are transient — stay silent, let onend restart
        };
        rec.start();
      } catch {

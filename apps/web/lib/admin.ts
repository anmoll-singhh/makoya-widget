...
export async function createCustomer(args: {
  ...
  return {
    email,
    tempPassword: created ? tempPassword : "(existing user — unchanged)",
    siteId: site.id,
    created,
  };
}
...
export async function listAdminSites(): Promise<AdminSiteRow[]> {
  ...
  const [{ data: sites, error }, emails] = await Promise.all([
    admin
      .from("sites")
      .select("id, owner_id, domain, plan, created_at")
...
export async function getAdminSiteDetail(siteId: string): Promise<AdminSiteDetail | null> {
  ...
  const { data: scans } = await admin
    .from("scans")
    .select("id, score, totals, created_at")
    .eq("site_id", siteId)
    .order("created_at", { ascending: false })
    .limit(20);
...
export async function listConsultationRequests(): Promise<AdminRequest[]> {
  ...
  return (reqs ?? []).map((r) => ({
    id: r.id,
    siteId: r.site_id,
    siteDomain: domainById.get(r.site_id) ?? "(deleted site)",
    type: r.type,
    status: r.status,
    note: r.note,
    createdAt: r.created_at,

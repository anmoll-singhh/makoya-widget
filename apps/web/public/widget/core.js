...
`,et='<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M6 6l12 12M18 6L6 18"/></svg>',j={"bottom-right":"bottom:16px; right:16px;","bottom-left":"bottom:16px; left:16px;","top-right":"top:16px; right:16px;","top-left":"top:16px; left:16px;"};function ot(){let t=null;const e=o=>{t&&(t.style.top=`${o.clientY}px`)};return{on(){t||(t=document.createElement("div"),t.setAttribute("aria-hidden","true"),t.style.cssText="position:fixed;left:0;width:100vw;height:28px;background:rgba(0,0,0,.06);border-top:2px solid rgba(0,0,0,.45);border-bottom:2px solid rgba(0,0,0,.45);pointer-events:none;z-index:2147483646;transform:translateY(-14px);",document.documentElement.appendChild(t),window.addEventListener("mousemove",e))},off(){window.removeEventListener("mousemove",e),t==null||t.remove(),t=null}}}function nt(t){var D,P;const e=document.createElement("div");e.id="makoya-widget-root";const o=e.attachShadow({mode:"open"});document.documentElement.appendChild(e);const r=document.createElement("style");r.textContent=tt(t.primaryColor),o.appendChild(r);const n=O(),s=ot(),l=(D=j[t.position])!=null?D:j["bottom-right"],i=document.createElement("button");i.className="mky-btn",i.style.cssText=l,i.setAttribute("aria-label","Accessibility options"),i.setAttribute("aria-expanded","false"),i.innerHTML=(P=I[t.launcherIcon])!=null?P:I.accessibility;const c=document.createElement("div");c.className="mky-panel",c.setAttribute("role","dialog"),c.setAttribute("aria-modal","true"),c.setAttribute("aria-label","Accessibility options"),c.style.cssText=l,c.style.bottom=t.position.startsWith("bottom")?"84px":"",c.style.top=t.position.startsWith("top")?"84px":"";const v=()=>{R(n),n.ruler?s.on():s.off(),K(n)},M=document.createElement("div");M.className="mky-head";const F=document.createElement("div"),S=document.createElement("h2");S.className="mky-title",S.textContent="Accessibility";const L=document.createElement("p");L.className="mky-sub",L.textContent="Adjust this page to your needs",F.append(S,L);const g=document.createElement("button");g.className="mky-close",g.type="button",g.setAttribute("aria-label","Close accessibility options"),g.innerHTML=et,M.append(F,g);const k=document.createElement("div");k.className="mky-body";function it(a){Object.assign(n,x),a.apply(n),v(),A()}function A(){k.innerHTML="";const a=document.createElement("div");a.className="mky-sec-label",a.textContent="Quick profiles";const p=document.createElement("div");p.className="mky-profiles";for(const f of X){const d=document.createElement("button");d.type="button",d.className="mky-chip",d.innerHTML=`${f.icon}<span>${f.label}</span>`,d.addEventListener("click",()=>it(f)),p.appendChild(d)}const b=document.createElement("div");b.className="mky-divider",k.append(a,p,b);for(const f of t.featuresEnabled){const d=Z[f];d&&k.appendChild(d(n,v))}}A();const w=document.createElement("div");w.className="mky-foot";const y=document.createElement("button");y.className="mky-reset",y.type="button",y.textContent="Reset all",y.addEventListener("click",()=>{Object.assign(n,x),v(),A()});const z=document.createElement("p");if(z.className="mky-note",z.textContent="Changes affect your view only — they don't alter the website.",w.append(y,z),!t.hideBranding){const a=document.createElement("p");a.className="mky-brand",a.innerHTML=`Powered by <a href="${t.brandingUrl}" target="_blank" rel="noopener">Makoya</a>`,w.appendChild(a)}c.append(M,k,w);let N=!1;const B=a=>{N=a,c.classList.toggle("open",a),i.setAttribute("aria-expanded",String(a)),a?g.focus():i.focus()};i.addEventListener("click",()=>B(!N)),g.addEventListener("click",()=>B(!1)),o.addEventListener("keydown",a=>{const p=a;if(N){if(p.key==="Escape"){B(!1);return}if(p.key==="Tab"){const b=Array.from(c.querySelectorAll('button, a[href], [tabindex]:not([tabindex="-1"])'));if(b.length===0)return;const f=b[0],d=b[b.length-1],_=o.activeElement;p.shiftKey&&_===f?(p.preventDefault(),d.focus()):!p.shiftKey&&_===d&&(p.preventDefault(),f.focus())}}}),o.append(i,c),v()}let $=!1;function rt(){const t=()=>R(O()),e=o=>{const r=history[o];history[o]=function(...n){const s=r.apply(this,n);return setTimeout(t,50),s}};e("pushState"),e("replaceState"),window.addEventListener("popstate",()=>setTimeout(t,50))}function C(t){if($||document.getElementById("makoya-widget-root"))return;$=!0;const e=V(t.siteId,t),o=()=>{nt(e),rt()};document.readyState==="loading"?document.addEventListener("DOMContentLoaded",o,{once:!0}):o()}return window.MakoyaWidget={init:C},function(){var e;try{const o=(e=document.currentScript)!=null?e:document.querySelector("script[data-site]");if(!o||o.hasAttribute("data-no-auto"))return;const r=o.dataset.site||"auto",n=o.dataset.color;C({siteId:r,...n?{primaryColor:n}:{}})}catch{}}(),E.init=C,Object.defineProperty(E,Symbol.toStringTag,{value:"Module"}),E}({});

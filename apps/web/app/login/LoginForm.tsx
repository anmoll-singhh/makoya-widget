...
export function LoginForm() {
  ...
  async function createAccount(e: React.MouseEvent) {
    ...
    if (signUpError) {
      setError(
        "Account creation failed. Please try again or sign in if you already have an account."
      );
      return;
    }
    if (data.session) {
  ...
  return (
    <>
      <form onSubmit={signIn} noValidate>
        {/* Email */}
        <label className="login-label" htmlFor="lf-email">
          Work email
        </label>
        ...
        {/* OR divider */}
        <div className="login-div" role="separator" aria-hidden="true">
          or
        </div>
